<?php
// source: /Applications/MAMP/htdocs/test/app/config/config.neon 
// source: /Applications/MAMP/htdocs/test/app/config/config.local.neon 

class Container_dc08bd2b06 extends Nette\DI\Container
{
	protected $meta = [
		'types' => [
			...
			'Nette\Security\IAuthenticator' => [1 => ['25_App_Model_UserManager']],
			'App\Model\UserManager' => [1 => ['25_App_Model_UserManager']],
			...
			'App\Presenters\ForumPresenter' => [1 => ['application.3']],
			'App\Presenters\HomepagePresenter' => [1 => ['application.4']],
			'App\Presenters\PostPresenter' => [1 => ['application.5']],
			'App\Presenters\ProfilePresenter' => [1 => ['application.6']],
	...
	public function __construct(array $params = [])
	{
		...
		$this->parameters += [
			'appDir' => '/Applications/MAMP/htdocs/test/app',
			'wwwDir' => '/Applications/MAMP/htdocs/test/www',
			'debugMode' => true,
			'productionMode' => false,
			'consoleMode' => false,
			'tempDir' => '/Applications/MAMP/htdocs/test/app/../temp',
		];
	}
	...
	public function createService__25_App_Model_UserManager(): App\Model\UserManager
	{
	...
	public function createServiceApplication__3(): App\Presenters\ForumPresenter
	{
		$service = new App\Presenters\ForumPresenter($this->getService('database.default.context'));
		$service->injectPrimary($this, $this->getService('application.presenterFactory'),
			$this->getService('routing.router'), $this->getService('http.request'),
			$this->getService('http.response'), $this->getService('session.session'),
			$this->getService('security.user'), $this->getService('latte.templateFactory'));
	...
	public function createServiceApplication__5(): App\Presenters\PostPresenter
	{
		$service = new App\Presenters\PostPresenter($this->getService('database.default.context'),
			$this->getService('security.user'), $this->getService('25_App_Model_UserManager'));
		$service->injectPrimary($this, $this->getService('application.presenterFactory'),
			$this->getService('routing.router'), $this->getService('http.request'),
			$this->getService('http.response'), $this->getService('session.session'),
			$this->getService('security.user'), $this->getService('latte.templateFactory'));
	...
	public function createServiceApplication__6(): App\Presenters\ProfilePresenter
	{
		$service = new App\Presenters\ProfilePresenter($this->getService('database.default.context'),
	...
	{
		$service = new Nette\Application\PresenterFactory(new Nette\Bridges\ApplicationDI\PresenterFactoryCallback($this, 5, '/Applications/MAMP/htdocs/test/app/../temp/cache/Nette%5CBridges%5CApplicationDI%5CApplicationExtension'));
	...
	{
		$service = new Nette\Caching\Storages\SQLiteJournal('/Applications/MAMP/htdocs/test/app/../temp/cache/journal.s3db');
		return $service;
	}
	...
	{
		$service = new Nette\Caching\Storages\FileStorage('/Applications/MAMP/htdocs/test/app/../temp/cache',
	...
	{
		$service = new Nette\Database\Connection('mysql:host=127.0.0.1;dbname=users;port=8889',
			'root', 'root', ['lazy' => true]);
	...
	public function createServiceLatte__latteFactory(): Nette\Bridges\ApplicationLatte\ILatteFactory
	{
		return new class ($this) implements Nette\Bridges\ApplicationLatte\ILatteFactory {
			...
			public function __construct(Container_dc08bd2b06 $container)
			{
			...
			public function create(): Latte\Engine
			{
				...
				$service->setTempDirectory('/Applications/MAMP/htdocs/test/app/../temp/cache/latte');
	...
	{
		$service = new Nette\Security\User($this->getService('security.userStorage'), $this->getService('25_App_Model_UserManager'));
